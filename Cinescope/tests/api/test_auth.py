class TestAuthAPI:
    ...
    def test_negative_wrong_pass(self, test_user, api_manager):
        wrong_user = test_user.model_copy(update={"password": "qwerty"})
        wrong_user_data = {
            "email": wrong_user.email,
            "password": wrong_user.password
        }

        response = api_manager.auth_api.login_user(wrong_user_data, expected_status=401)
        response_data = response.json()

        assert "error" in response_data

    def test_negative_wrong_email(self, test_user, api_manager):
        wrong_user = test_user.model_copy(update={"email": "qwerty"})
        wrong_user_data = {
            "email": wrong_user.email,
            "password": wrong_user.password
        }
